...
// Si estamos en localhost, usamos la API local; si no, la de producción
const BASE_URL = 'https://api.pharmacontrol.site';
...
const scheduleRefresh = (t) => {
  ...
  if(!exp) return;
  const ms = exp*1000 - Date.now() - 30000;
...
async function loginFront(email, pass){
  ...
  if (!r.ok) {
    if (r.status === 500) {
      throw new Error('Error del servidor (500). Revisa los logs de la API.');
    }
...
async function logoutFront(){
  ...
  fetch(BRIDGE.clear,{method:'POST'}).catch(()=>{});
}
...
function bindLoginPage(){
  ...
  const go = async() => {
    try {
      ...
      await loginFront((email.value||'').trim(), (pass.value||'').trim());
      location.href='/medicamentos/';
    } catch(e){
      if(msg){
        msg.textContent = e.message || 'Error de login';
        msg.style.display = '';
      }
    }
  };
  ...
  [email, pass].forEach(i=>i?.addEventListener('keydown', e=>{ if(e.key==='Enter') go(); }));
}
